...
public class IsoSpriteSortingManager : Singleton<IsoSpriteSortingManager> 
{
    ...
    //Relate the new sprite its overlapped sprites if necessary and register it
    ...
    //If the newSprite is overlapped by any of the existing static sprites, register them to each other in relation to depth
    private static void SetupStaticDependencies(IsoSpriteSorting newSprite) 
    {
        ...
        for (int i = 0; i < theCount; i++) 
        {
            ...
            if (b1.minX <= b2.maxX && b2.minX <= b1.maxX && b1.maxY >= b2.minY && b2.maxY >= b1.minY) 
            { //Inline Bounds2D.Intersects
    ...
    //Identify what list the sprite (IsoSpriteSorting) is classified under and remove it
    ...
    //Remove sprite (IsoSpriteSorting) classified as a static world object from itself from the registries of its overlaping(?) static world objects
    ...
    //Updates sort of objects for each frame
    ...
    //Check each object for if it should refresh its cached data
    ...
    //Register each sprite that overlaps with a sprite in the moveableList based on their depth relationship
    ...
    //Clear each sprite's (IsoSpriteSorting) movingDependencies
    ...
    //Check if the two sprite's bounds intersect
    ...
    //Adds the sorted list to the rendering registery in render order
    ...
    //Set each sprite (IsoSpriteSorting) to a decreasingly negative sorting order, setting the furthest sprites to render furthest away (?) 
    ...
    //Clear the destinationList and add each visible sprite in the fullList to the destinationList
    ...
    //Order sprites (IsoSpriteSorting) by their y point/line, lowest to highest
    private static void SortListSimple(List<IsoSpriteSorting> list) 
    {
        list.Sort((a, b) => !a || !b ? 0 : IsoSpriteSorting.CompareIsoSorters(a, b));
    }
}
