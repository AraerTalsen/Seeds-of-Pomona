...
{
    //Literally an object in world that can move
    public bool isMovable;
    [SerializeField] private bool renderBelowAll;
    public bool RenderBelowAll { get => renderBelowAll; set => renderBelowAll = value; }

    [NonSerialized] public bool registered = false;
    [NonSerialized] public bool forceSort;

    //Overlapping static in-world objects, the objects that are further than `this`
    [NonSerialized] public readonly List<IsoSpriteSorting> staticDependencies = new List<IsoSpriteSorting>(16);
    //Overlapping static in-world objects that reference `this`, the objects that are closer than `this`
    [NonSerialized] public readonly List<IsoSpriteSorting> inverseStaticDependencies = new List<IsoSpriteSorting>(16);
    //Overlapping movable in-world objects
    [NonSerialized] public readonly List<IsoSpriteSorting> movingDependencies = new List<IsoSpriteSorting>(8);
    ...
    //If there are renderers to sort, recalculate this object's sorting points and sprite bounding box
    ...
    //Find the mid point of the sorting line
    ...
    //Find mid point of the HEIGHT of the sorting line
    ...
    //Check if cache should be refreshed and do so
    ...
    //Check if the transform or any of its children have changed since the last time its flag was set to false
    ...
    //Render order has changed between update and late update
    public void LateUpdateHasChanged() 
    {
        if (t.hasChanged && lastRefreshedFrame == Time.frameCount) 
        { //If it didn't refresh this frame, the transform may have moved between Update and LateUpdate
            t.hasChanged = false;
        }
    }
    ...
    //Store reference to every renderer on this object and its children
    ...
    //Segment line1 is above segment line2 = -1
    //Segment line1 is below segment line2 = 1
    ...
    //Mid height of segment line1 is below mid height of segment line2 = 1
    //Mid height of segment line1 is above mid height of segment line2 = -1
    //Mid height of segment line1 is on mid height of segment line2 = 0
    ...
    //Point above infinite line = -1
    //Point on or below infinite line = 1
    ...
    //Unregister object from the manager and any relevent dependencies
